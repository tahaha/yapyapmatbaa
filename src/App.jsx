...
const steps = [
  { number: '01', title: 'Bize ulasin', text: 'WhatsApp\'tan bize ulasin ve bilgilerinizi gonderin.', icon: MessageCircle },
  { number: '02', title: 'Tasarimi onaylayin', text: 'Ucretsiz tasarim destegimizle taslaginizi hazirlayip onayiniza sunalim.', icon: Palette },
  { number: '03', title: 'Kapiniza gelsin', text: 'Hizlica baskiya alip kargo ile size ulastiralim.', icon: Truck },
];
        ...
        <div className="relative mx-auto grid max-w-6xl items-center gap-14 px-5 sm:px-8 lg:grid-cols-[1.08fr_.92fr] lg:gap-20">
          <div className="max-w-3xl">
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-[#54e98f]/25 bg-[#17c964]/10 px-3.5 py-2 text-xs font-bold text-[#75f0a5] sm:text-sm">
              <Sparkles size={16} /> Tasarim Olusturmak Icin Ucretsiz Tasarim Destegi!
            </div>
            <h1 className="text-[2.55rem] font-black leading-[1.05] tracking-[-0.055em] sm:text-6xl lg:text-[4.35rem]">
              Markanizi Yansitan, <span className="text-[#54e98f]">Akilda Kalici</span> Kartvizit ve Baski Tasarimlari
            </h1>
            <p className="mt-6 max-w-xl text-lg leading-relaxed text-slate-300 sm:text-xl">
              Siz hayal edin, biz tasarlayip kapiniza kadar gondeRelim.
            </p>
            <div className="mt-8 flex flex-col items-start gap-4 sm:flex-row sm:items-center">
              ...
              >
                <MessageCircle size={22} />Hemen WhatsApp'tan Bilgi Al
                <ArrowRight size={19} className="transition-transform group-hover:translate-x-1" />
              </a>
              <span className="flex items-center gap-2 text-sm font-medium text-slate-400">
                <Check size={17} className="text-[#54e98f]" /> Hizli yanit · Ucretsiz teklif
              </span>
            </div>
          </div>
              ...
              <div className="rounded-[1.4rem] bg-[#edf1ee] p-6 text-[#102331]">
                <div className="mb-10 flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-black tracking-[-0.04em]">
                      Iyi tasarim,<br />iyi bir iz birakir.
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-2xl bg-white p-4 shadow-sm">
                    ...
                    <p className="text-sm font-extrabold">Ozgun<br />tasarim</p>
                  </div>
                  <div className="rounded-2xl bg-[#17c964] p-4">
                    <BadgeCheck className="mb-5" size={24} />
                    <p className="text-sm font-extrabold">Kaliteli<br />baski</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <section id="hizmetler" className="scroll-mt-8 py-20 sm:py-28">
        <div className="mx-auto max-w-6xl px-5 sm:px-8">
          <div className="mb-10 flex flex-col justify-between gap-5 sm:mb-14 sm:flex-row sm:items-end">
            <div>
              <p className="mb-3 text-xs font-black uppercase tracking-[.18em] text-[#11984b]">Urunlerimiz</p>
              <h2 className="text-3xl font-black tracking-[-0.045em] text-[#071b2b] sm:text-5xl">Ihtiyaciniza uygun baskiyi secin.</h2>
            </div>
            <div className="sm:text-right">
              <p className="max-w-md text-sm leading-relaxed text-slate-500">
                Her urunun ebat, adet ve uretim seceneklerini inceleyin; fiyati seciminize gore gorun.
              </p>
              <a href={productsHref} className="mt-3 inline-flex items-center gap-1.5 text-sm font-extrabold text-[#11984b] hover:text-[#071b2b]">
                Tum urunleri gor <ArrowRight size={16} />
              </a>
            </div>
          </div>
      ...
      <section id="surec" className="scroll-mt-8 bg-white py-20 sm:py-28">
        <div className="mx-auto max-w-6xl px-5 sm:px-8">
          <div className="mx-auto mb-12 max-w-2xl text-center">
            <p className="mb-3 text-xs font-black uppercase tracking-[.18em] text-[#11984b]">Surec</p>
            <h2 className="text-3xl font-black tracking-[-0.045em] text-[#071b2b] sm:text-5xl">Nasil calisiyoruz?</h2>
            <p className="mt-4 text-slate-500">Fikirden teslimata, yalnizca uc kolay adim.</p>
          </div>
          <div className="relative grid gap-5 md:grid-cols-3">
            ...
            {steps.map((step) => {
              ...
              return (
                <article key={step.number} className="relative rounded-3xl border border-slate-100 bg-[#f8faf9] p-6 sm:p-7">
                  <div className="relative mb-7 flex items-center justify-between">
                    <span className="grid h-14 w-14 place-items-center rounded-2xl bg-[#071b2b] text-[#54e98f] shadow-lg">
                      <Icon size={25} />
                    </span>
                    <span className="text-4xl font-black tracking-[-0.07em] text-slate-200">{step.number}</span>
                  </div>
            ...
            <div className="grid gap-10 lg:grid-cols-[1fr_auto] lg:items-center">
              <div>
                <div className="mb-4 inline-flex items-center gap-2 text-sm font-bold text-[#54e98f]">
                  <BadgeCheck size={18} /> Tasarim destegi ucretsiz
                </div>
                <h2 className="max-w-2xl text-3xl font-black leading-tight tracking-[-0.045em] sm:text-5xl">
                  Markanizi baskida one cikarmaya hazir misiniz?
                </h2>
                <p className="mt-4 max-w-xl leading-relaxed text-slate-300">
                  Ihtiyacinizi anlatin, size en uygun urun ve tasarim icin hizlica yardimci olalim.
                </p>
              </div>
              <a
                href={createWhatsAppUrl()}
              ...
              >
                <Send size={20} /> Teklif Iste <ArrowRight size={18} className="transition-transform group-hover:translate-x-1" />
              </a>
            </div>
          </div>
        </div>
